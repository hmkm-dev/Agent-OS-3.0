...
## Runtime durability + web capability hardening

- Trajectory events and checkpoints now have durable PostgreSQL adapters and are wired into mission dispatch/verification/failure paths.
- Scheduler metadata is persisted in PostgreSQL using stable callback keys; arbitrary Python callbacks are never serialized. Unknown callbacks are skipped on restart.
- Crawl4AI is exposed as an optional MCP web-crawling capability; Playwright remains the interactive browser capability.
- Agent Reach is exposed through an explicit CLI adapter. The gateway fails closed when the CLI is unavailable and never fabricates a result.
